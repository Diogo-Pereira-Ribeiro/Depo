...
def main():
    ...
    for ext in extensions_only:
        if 'apk' in ext and ext['apk']:
            ...
            if not str(ext['apk']).startswith('http'):
                ext['apk'] = raw_base_repo + str(ext['apk']).lstrip('/')
        if 'icon' in ext and ext['icon']:
            if not str(ext['icon']).startswith('http'):
                # icons served from main/icon/ so they are immediately available
                ext['icon'] = raw_base_main + str(ext['icon']).lstrip('/')
